const img = (name) => `../assets/images/welcome/${name}`;
const mural = [
...
const videoBase = '../assets/video/';
const video=[['cat_video.webp','反弹琵琶乐舞','02:18',videoBase + '敦煌反弹琵琶乐舞 .mp4'],['featured_pipa.webp','飞天舞姿','01:56',videoBase + '飞天舞姿.mp4'],['graceful_dance_among_ancient_temples.webp','敦煌舞动作展示','03:24',videoBase + '敦煌舞动作展示.mp4'],['pipa_dance.webp','壁画动作复原','02:47',videoBase + '壁画动作复原.mp4']];
document.getElementById('videoGrid').innerHTML = video.map(([im,t,time,src])=>`<div class="video-card" role="button" tabindex="0" aria-label="播放${t}" data-video="${src}" data-title="${t}"><img src="${img(im)}" onerror="this.src='${img('cat_video.webp')}'"><b>${t}</b><small>${time}</small></div>`).join('');
...
videoOverlay.innerHTML = `
  <div class="video-popup">
    ...
    <p class="video-title-label"></p>
  </div>
`;
document.body.appendChild(videoOverlay);

const videoPlayer = videoOverlay.querySelector('.video-player');
const videoTitleLabel = videoOverlay.querySelector('.video-title-label');
